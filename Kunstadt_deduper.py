...
with open(args.file, 'r') as infile, open(args.outfile, 'w') as outfile:
    ...
    while True:
        ...
        if line == "": # Break if EOF
            ...
            # Could not get this working in time, wanted to wipe each chromosome but this just kept causing issues. 
            # if curr_chrom == "":
            #     curr_chrom = chrom
            # elif str(chrom) != str(curr_chrom):
            #     dupset = set() # Wipe the dupset every new chromosome to save on memory
            #     print ("Curr chrom:"+ curr_chrom + '\n')
            #     print ("Chrom:" + chrom +'\n')
            #     curr_chrom = chrom
...
print("Duplicate Reads: " + str(dup_count))
